...
MemoCat is a local-first, self-hosted MCP shared-memory server maintained by
MontyGovernance. It can be used by Claude Desktop, OpenAI Codex, Cursor, and
other MCP-compatible systems. This policy describes data handling by MemoCat,
...
MemoCat does not intentionally collect conversations or files outside the
values an AI system sends in an invoked tool call. MemoCat does not include
...
`memocat_remove_keyspace`. Removing MemoCat from an MCP client, including
