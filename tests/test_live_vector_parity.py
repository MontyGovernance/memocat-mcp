...
async def test_external_vectors_write_search_update_and_status(server, keyspace):
    ...
    payload(await server.montycat_disable_semantic(
        keyspace=keyspace, drop_vectors=True
    ))
    enrolled = await server.montycat_enable_external_vectors(
        keyspace=keyspace,
        dimensions=3,
        embedding_space="montycat-mcp-live-test:v1",
    )
    payload(enrolled)

    stored = payload(await server.montycat_remember(
        keyspace=keyspace,
        value={"text": "vector memory"},
        vector=[1.0, 0.0, 0.0],
    ))

    payload(await server.montycat_update(
        keyspace=keyspace,
        key=str(stored),
        updates={"state": "updated"},
        vector=[0.0, 1.0, 0.0],
    ))
    payload(await server.montycat_remember_bulk(
        keyspace=keyspace,
    ...
    status = payload(await server.montycat_semantic_status(keyspace=keyspace))
...
async def test_reembed_semantic_is_available_through_mcp(server, keyspace):
    ...
    payload(await server.montycat_enable_semantic(
        keyspace=keyspace,
        semantic_model="bge-small",
    ))
    result = await server.montycat_reembed_semantic(
        keyspace=keyspace,
        semantic_model="bge-small",
    )
    payload(result)
